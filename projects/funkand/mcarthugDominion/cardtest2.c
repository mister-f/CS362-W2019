 ...
 * Author: Andrew Funk
 * Date: 02-09-19
 * Description: A unit test function for the dominion.c adventurer card
...
int main() {
	...
	assertTrue(countTreasure(&G, 0), countTreasure(&GCopy, 0) + 2);

	printf("TEST: Treasure cards come from own deck.\n");
	assertTrue(G.deckCount[0], GCopy.deckCount[0] - G.discardCount[0] - 2);
	...
	printf("TEST: Drawn non-treasure cards are discarded.\n");
	assertTrue(G.deckCount[0] + G.handCount[0] + G.discardCount[0],
				GCopy.deckCount[0] + GCopy.handCount[0]);	
