 ...
 * Author: Andrew Funk
 * Date: 02-24-19
 * Description: A random test function for the dominion.c smithy card
...
int main() {
	...
	for(j = 0; j < 1000; j++)
	{
		...
		assertTrue(G.handCount[randPlayer], GCopy.handCount[randPlayer] + 2);

		printf("TEST: 3 player cards come from own deck.\n");
		assertTrue(G.deckCount[randPlayer], GCopy.deckCount[randPlayer] - 3);

		printf("TEST: Smithy card no longer in hand after being played.\n");
		handPos = -1;
		if(G.hand[randPlayer][0] == smithy)
		{
			handPos = 0;
		}
