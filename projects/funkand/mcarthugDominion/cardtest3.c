 ...
 * Author: Andrew Funk
 * Date: 02-09-19
 * Description: A unit test function for the dominion.c great hall card
...
int main() {
	...
	cardEffect(great_hall, 0, 0, 0, &G, handPos, &bonus);

	printf("TEST: Player receives 1 card to hand.\n");
	// We don't add +1 to GCopy count  here because great_hall is discarded
	assertTrue(G.handCount[0], GCopy.handCount[0]);

	printf("TEST: Player card comes from own deck.\n");
	assertTrue(G.deckCount[0], GCopy.deckCount[0] - 1);

	printf("TEST: Action number is increased by one.\n");
	assertTrue(G.numActions, GCopy.numActions + 1);

	printf("TEST: Great Hall card no longer in hand after being played.\n");
	handPos = -1;
	for(i = 0; i < G.handCount[0]; i++)
	{
		if(G.hand[0][i] == great_hall)
